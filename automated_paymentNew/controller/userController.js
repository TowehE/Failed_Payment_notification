...
//function to upload image
  exports.uploadImage = async (req, res) => {
    try {
        ...
        let fileUploader;
        try {
            fileUploader = await uploadImageToCloudinary(profilePicture, user);
        ...
        if (fileUploader) {
            ...
            const updateduserPicture = await userModel.findByIdAndUpdate(userId, { profilePicture: userPicture }, { new: true });
  
            return res.status(200).json({
                message: 'Profile picture uploaded successfully',
                data: {updateduserPicture}
            });
        } else {
            return res.status(500).json({
